...
class SystemMenu extends Model
{
     ...
     * @return array
     */
    public static function getBreadCrumbs($ids)
    {
        $menu = Cache::get('admin_bread_crumbs_'.ADMIN_ID);
        $menu = self::where([['id', 'in', $ids]])->field('id,pid,title,url,param')->select();
        Cache::set('admin_bread_crumbs_'.ADMIN_ID, $menu);
        return $menu;
    }
    ...
    public static function importMenu($data = [], $mod = '', $type = 'module', $pid = 0)
    {
        ...
        if ($type == 'module') {// 模型菜单
            ...
            foreach ($data as $v) {
                ...
                if (isset($v['is_menu']) && (int)$v['is_menu'] === 0) {
                    continue;
                }
     ...
     * @since 2020-12-06 18:16:35
     */
    public static function getParentsArr($id = 0,$field='id,pid'){
        static $res;
        $menu = self::where('id='.$id)->field($field)->find();
        // var_dump($menu);
        if($menu && $menu['pid']>0){
            $res[] = $menu['id'];
            self::getParentsArr($menu['pid']);
        }else{
            $res[] = $menu['id'];
        }
